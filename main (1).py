...
def _normalise_phone(value) -> str:
    """Normalise a phone number to E.164 (+61... for AU).
    Accepts: +61400000000, 0400000000, 61400000000, with spaces/dashes/brackets.
    Returns the cleaned international form; passes through any other format
    ...
    if cleaned.startswith("+"):
        return cleaned
    # AU local form: 04xxxxxxxx, 02xxxxxxxx, 03xxxxxxxx, 07xxxxxxxx, 08xxxxxxxx
    if cleaned.startswith("0") and len(cleaned) == 10:
        return "+61" + cleaned[1:]
    # AU national without leading +: 614xxxxxxxx
    if cleaned.startswith("61") and len(cleaned) == 11:
        return "+" + cleaned
    # Pure digits, can't tell — prefix with + and let downstream reject if invalid
    if cleaned.isdigit():
        return "+" + cleaned
    return cleaned
